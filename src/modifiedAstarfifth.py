...
def astarModified(MAZE, Dim, x, y):
    """
    ...
    open_list.add((0,(maze[x][y].fn, maze[x][y].hn), (x, y), maze[x][y].parent))
    ...
    while open_list:
        ...
        # inverted list.
        if i1 == dim - 1 and j1 == dim - 1:  # maze[i1][j1].state == 100:
            path = list()
            while (i1, j1) != (x, y):
                path.append((i1, j1))
        ...
        for (X, Y) in child_array:
            ...
            if a + b > 0 and 0 <= a < dim and 0 <= b < dim:
                ...
                if maze[a][b].hidden is True:
                    probablity_block=calculateProbablityBlocked(a,b,maze)
                else:
                    if maze[a][b].state==1:
                        probablity_block=1
                    else:
                        probablity_block=0
                print(probablity_block)
                ...
                # first discard the old entry in open_list and then add the updated one.
                    ...
                    if maze[a][b].gn <= child_curr_cost:
                        ...
                        open_list.discard((probablity_block,(maze[a][b].fn, maze[a][b].hn), (a, b), maze[a][b].parent))
                        maze[a][b].gn = child_curr_cost
                        maze[a][b].fn = child_curr_cost + maze[a][b].hn
                        maze[a][b].parent = (i1, j1)
                        open_list.add((probablity_block,(maze[a][b].fn, maze[a][b].hn), (a, b), (i1, j1)))
...
def calculateProbablityBlocked(x,y,new_maze):
    total_probablity=0
    print((x,y))
    print(new_maze[x][y].probablity_blocked)
    for neigh in new_maze[x][y].probablity_blocked:
        total_probablity=total_probablity+(new_maze[neigh[0]][neigh[1]].cx - new_maze[neigh[0]][neigh[1]].bx)/new_maze[neigh[0]][neigh[1]].hx
